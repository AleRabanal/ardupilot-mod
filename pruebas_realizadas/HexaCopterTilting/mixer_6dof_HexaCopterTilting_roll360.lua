...
local VELOCIDAD_ROLL_DEG_S = 10.0  -- Velocidad de giro en grados por segundo (a 45°/s tardará 8 segundos en dar la vuelta completa)
...
function calc_lateral(x, y, spin)
    ...
    local forward = -sphi *4.0
    local lateral =  cphi *4.0
...
function cargar_parametros()
    ...
    param:set_and_save('ATC_RAT_RLL_P', 0.03)
    param:set_and_save('ATC_RAT_RLL_I', 0.03)
    param:set_and_save('ATC_RAT_RLL_D', 0.0)

    param:set_and_save('ATC_RAT_PIT_P', 0.03)
    param:set_and_save('ATC_RAT_PIT_I', 0.03)
