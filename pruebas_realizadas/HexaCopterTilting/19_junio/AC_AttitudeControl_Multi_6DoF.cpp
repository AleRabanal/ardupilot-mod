...
void AC_AttitudeControl_Multi_6DoF::rate_controller_run() {
    ...
    // Forzamos internamente a que la bandera de vuelo invertido de la clase base 
    // se entere de que estamos rotando intencionadamente si el offset pasa de 90º.
    //_inverted_flight = (fabsf(roll_offset_deg) > 90.0f);
...
void AC_AttitudeControl_Multi_6DoF::input_euler_angle_roll_pitch_euler_rate_yaw(float euler_roll_angle_cd, float euler_pitch_angle_cd, float euler_yaw_rate_cds) {
    auto& non_const_aparm = const_cast<AP_MultiCopter&>(_aparm);
    non_const_aparm.angle_max.set(8000.0f);
...
void AC_AttitudeControl_Multi_6DoF::thrust_vector_rotation_angles(const Quaternion& attitude_target, const Quaternion& attitude_body, Quaternion& thrust_vector_correction, Vector3f& attitude_error, float& thrust_angle, float& thrust_error_angle) const
{
    // 1. El vector original apunta hacia abajo del dron [0,0,-1]
    Vector3f thrust_vector_up{0.0f, 0.0f, -1.0f};

    // ---- NUESTRO DETECTOR Y BYPASS PARA VUELO INVERTIDO 6DoF ----
    // Obtenemos la rotación actual para saber si el chasis pasó los 90 grados.
    // En la matriz de rotación Body-to-NED, c.z es el coseno del ángulo de inclinación total.
    // Si c.z < 0, el dron está boca abajo (>90°).
    float cos_tilt = AP::ahrs().get_rotation_body_to_ned().c.z;
    
    if (cos_tilt < 0.0f) {
        // ¡Boca abajo! Engañamos a las matemáticas del PID haciéndole creer que 
        // el vector de empuje natural del cuerpo ahora apunta hacia el "techo" del dron [0,0,1].
        // De esta forma, el PID buscará la estabilidad en posición invertida 
        // y calculará empuñaduras/errores realistas y pequeños para tu Control Allocation.
        thrust_vector_up = Vector3f(0.0f, 0.0f, 1.0f);
    }
    // -------------------------------------------------------------

    // A partir de aquí, el código corre EXACTAMENTE igual que el original de ArduPilot,
    // pero utilizando nuestro 'thrust_vector_up' dinámico:
