...
/*
// ← LUEGO el wrench y la multiplicación
const float F_max       = 12.75f;
const float T_max_roll  = F_max * 0.275f;
const float T_max_pitch = F_max * 0.238f;
const float T_max_yaw   = 0.15f * F_max;

wrench[0] = roll_thrust    * T_max_roll;
wrench[1] = pitch_thrust   * T_max_pitch;
wrench[2] = yaw_thrust     * T_max_yaw;
wrench[3] = forward_thrust * F_max;
wrench[4] = right_thrust   * F_max;
wrench[5] = throttle_thrust * 24.5f;  // peso real del dron en N
...
*/
        ...
        AP::logger().Write( 
    "TMS", "TimeUS,T1,T2,T3,T4,T5,T6", "Qffffff", 
    AP_HAL::micros64(),
        roll_thrust, 
        pitch_thrust, 
        yaw_thrust, 
    ...
    // Rotar el throttle demand a body frame
   /* Matrix3f rot;
    rot.from_euler312(_roll_offset, _pitch_offset, 0.0f);
    Vector3f throttle_vec(forward_thrust, right_thrust, throttle_thrust);
    throttle_vec = rot * throttle_vec;

    wrench[0] = roll_thrust;
    wrench[1] = pitch_thrust;
    wrench[2] = yaw_thrust;
    wrench[3] = throttle_vec.x;  // forward thrust in body frame
    wrench[4] = throttle_vec.y;  // right thrust in body frame
    wrench[5] = throttle_vec.z;  // throttle thrust in body frame 
    */


        
    // 1. Calculamos la rotación clásica tal y como la tienes
    Matrix3f rot;
    rot.from_euler312(_roll_offset, _pitch_offset, 0.0f);
    Vector3f throttle_vec(forward_thrust, right_thrust, throttle_thrust);
    throttle_vec = rot * throttle_vec;
    ...
    wrench[1] = pitch_thrust;
    wrench[2] = yaw_thrust;
    wrench[3] = forward_thrust;  // forward thrust
    wrench[4] = right_thrust;  // right thrust
    wrench[5] = throttle_thrust;  // throttle thrust

    AP::logger().Write( 
    "WR", "TimeUS,T1,T2,T3,T4,T5,T6", "Qffffff", 
    AP_HAL::micros64(),
        wrench[0], 
        wrench[1], 
        wrench[2], 
...
void AP_MotorsMatrix_6DoF_Scripting::build_effectiveness_matrix()
{
    ...
    for (uint8_t i = 0; i < 6; i++) {
        ...
        const float yaw_scale   = 4.636f; //1.0f / 0.275f;
