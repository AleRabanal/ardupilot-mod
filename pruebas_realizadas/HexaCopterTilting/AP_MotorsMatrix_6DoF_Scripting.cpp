    ...
    switch (_spool_state) {
        ...
        case SpoolState::GROUND_IDLE:
        {
            ...
            for (uint8_t i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
                if (motor_enabled[i]) {
                    if (i >= 6) {
                        _actuator[i] = 0.5f; // 0.5 * 4500 = 2250 -> Centro exacto (1500us)
                    } else {
        ...
        case SpoolState::SPOOLING_DOWN:
            ...
            // EN VUELO: Interceptamos las 12 señales virtuales y las convertimos en físicas
            for (uint8_t i = 0; i < 6; i++) {
                float F_v = _thrust_rpyt_out[i];     // Fuerza Vertical Teórica (0..5)
                float F_l = _thrust_rpyt_out[i + 6]; // Fuerza Lateral Teórica (6..11)

                // --- COMPUTO DEL MOTOR REAL (0 al 5) ---
                float thrust_mod = safe_sqrt((F_v * F_v) + (F_l * F_l));
                thrust_mod = constrain_float(thrust_mod, 0.0f, 1.0f);
                
                // Se le aplica la curva de empuje y escala de voltaje nativa de ArduPilot
                _actuator[i] = thr_lin.thrust_to_actuator(thrust_mod);

                // --- COMPUTO DEL SERVO REAL (6 al 11) ---
                float angle_rad = 0.0f;
                if (is_positive(thrust_mod)) {
                    angle_rad = atan2f(F_l, F_v);
                }

                // Algoritmo Unwrap para las +-2 vueltas
                float diff = angle_rad - _last_servo_angle_rad[i];
                while (diff < -M_PI)  diff += 2.0f * M_PI;
                while (diff > M_PI)   diff -= 2.0f * M_PI;

                float unwrapped_angle = _last_servo_angle_rad[i] + diff;
                float max_limit_rad = 4.0f * M_PI; // +-720 grados
                unwrapped_angle = constrain_float(unwrapped_angle, -max_limit_rad, max_limit_rad);
                _last_servo_angle_rad[i] = unwrapped_angle;

                // Mapeo directo a rango [0.0, 1.0] donde 0.5 es el centro físico (1500us)
